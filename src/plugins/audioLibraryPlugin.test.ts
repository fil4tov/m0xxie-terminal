...
it("discovers audio recursively, derives exact filenames and encodes public URLs", async () => {
  ...
  const nested = join(audioDirectory, "album");
  mkdirSync(nested, { recursive: true });
  const sounds = join(directory, "sounds");
  mkdirSync(sounds);
  const excluded = [
    join(directory, "outside.mp3"),
    join(sounds, "play.mp3"),
    join(sounds, "pause.wav"),
  ];
  ...
  try {
    ...
    for (const file of excluded) writeFileSync(file, "");
    expect(await readAudioLibrary(directory, "/music/")).toEqual([
      {
        title: "01 — Песня #1",
        artist: "m0xxie",
        src: `/music/audio/album/${encodeURIComponent("01 — Песня #1.MP3")}`,
        duration: null,
      },
    ]);
    unlinkSync(audio);
    ...
    unlinkSync(ignored);
    for (const file of excluded) unlinkSync(file);
    rmdirSync(sounds);
    rmdirSync(nested);
    rmdirSync(audioDirectory);
    rmdirSync(directory);
  }
});
...
it("computes MP3 duration from frames at build time and refreshes changed files", async () => {
  ...
  const file = join(audioDirectory, "sample.mp3");
